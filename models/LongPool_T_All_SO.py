# 下采样过程，三次下采样，全部使用长条池化，池化核为（2,4,4），并添加侧输出（SideOutput,SO）
...
class Up3D(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.up = nn.Upsample(scale_factor=(2, 4, 4), mode='trilinear', align_corners=True)
...
class LongPool_T_All_SO(nn.Module):
    """
    ...
    def __init__(self, in_channels, out_channels):
        super(LongPool_T_All_SO, self).__init__()
        # encoder
        # self.down1 = PreProcess3D(in_channels, 64)
        # self.down2 = Down3D(64, 128)
        # self.down3 = Down3D(128, 256)
        # self.down4 = Down3D(256, 512)
        #
        # # decoder
        ...
        #
        # # side output
        # self.side_enc_4 = nn.Conv3d(512, out_channels, kernel_size=1)
        # self.side_dec_3 = nn.Conv3d(256, out_channels, kernel_size=1)
        # self.side_dec_2 = nn.Conv3d(128, out_channels, kernel_size=1)
        #
        # # out logits
