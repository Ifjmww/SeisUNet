# 下采样过程，前两次下采样使用长条池化，池化核为（4,2,2），第三次使用最大池化，并添加侧输出（SideOutput，SO）
...
class LongPool_Phase1_2_SO(nn.Module):
    """
    ...
    def __init__(self, in_channels, out_channels):
        super(LongPool_Phase1_2_SO, self).__init__()

        # encoder
        self.down1 = PreProcess3D(in_channels, 32)
        self.down2 = LongPool_Down3D(32, 64)
        self.down3 = LongPool_Down3D(64, 128)
        self.down4 = Down3D(128, 256)

        # decoder
        self.up1 = Up3D(256, 128)
        self.up2 = LongPool_Up3D(128, 64)
        self.up3 = LongPool_Up3D(64, 32)
...
if __name__ == '__main__':
    model = LongPool_Phase1_2_SO(3, 2).to('cuda:0')
