...
def save_per_result(gy, save_path, pred_data_name, figure_name):
    k1, k2, k3 = 99, 29, 29
    gy1 = gy[k1, :, :]
    gy2 = gy[:, k2, :]
    gy3 = gy[:, :, k3]
    ...
    plt.savefig(save_path + pred_data_name + '_' + figure_name + '.png', dpi=600)


def pred_Gaussian(args, model_path):
    ...
    save_path = './EXP/' + args.model_type + '/' + args.exp + '/results/pred/' + args.pred_data_name + '_circle/'
    ...
    figure_name = model_path.split('/')[-1]
    save_per_result(output_data, save_path + '/picture/', args.pred_data_name, figure_name)
    print("Finish!!!")


def pred_Circle(args):
    print("============================== pred_Gaussian ==============================")
    for item in os.listdir('./EXP/' + args.model_type + '/' + args.exp + '/models/'):
        print(item)
        model_path = './EXP/' + args.model_type + '/' + args.exp + '/models/' + item
        pred_Gaussian(args, model_path)
