...
#     weight_map = np.zeros(input_shape)
#
#     total_iterations = num_blocks[0] * num_blocks[1] * num_blocks[2]
#     progress_bar = tqdm(total=total_iterations, desc='[Pred]', unit='it')
#
...
#                 end = (start + block_shape).astype(int)
#                 # 裁剪当前块的数据
#                 block = input_data[start[0]:end[0], start[1]:end[1], start[2]:end[2]]
#
#                 # 进行预测（这里假设使用你的语义分割模型进行预测
#                 block = block.reshape((1, 1, block.shape[0], block.shape[1], block.shape[2]))
#                 block_mean = np.mean(block)
...
def display():
    gx = np.load('D:/Test/dataset/seismic/原始数据/各种三维断层数据/numpy/Kerry_mini3D_t480_c730_i286.npy')
    gy = np.load('F:/New_Test/U-Net_fault/EXP/AS_2/0805_d200G3_e30_AS2_L2dice1000_EXP2/results/pred/kerry_mini/numpy/kerry_mini.npy')

    k1, k2, k3 = 300, 150, 120
    gx1 = gx[k1, :, :]
    gy1 = gy[k1, :, :]
    gx2 = gx[:, k2, :]
    gy2 = gy[:, k2, :]
