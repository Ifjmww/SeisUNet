...
from utils.test_f3wu_Gaussian import pred_f3wu_Gaussian
from utils.test_Gaussian import pred_Gaussian
from utils.test_Circle import pred_Circle
...
def add_args():
    ...
    parser.add_argument("--lr_sc", default="Ex_LR", choices=['Ex_LR', 'Cos_LR'], type=str, help="optimization algorithm")
    ...
    parser.add_argument("--model_type", default='LP_All_SO',
                        choices=['LP_All', 'LP_All_SO', 'LP_T_All', 'LP_T_All_SO', 'MP_All_SO', 'LP_P1_SO', 'LP_P12_SO', 'LP_P3_SO'],
                        type=str,
                        help='network run mode')

    parser.add_argument('--pred_mode', default='Gauss', choices=['f3wu_Gauss', 'Gauss', 'Circle', 'SW'], type=str, help='network pred mode')
    parser.add_argument("--pred_data_name", default="kerry",
                        choices=['f3wu', 'kerry', 'kerry_mini', 'opunake', 'f3_2023_demo', 'f3_2023_demo_cut', 'parihaka_train', 'f3wang'], type=str,
...
def main(args):
    if args.mode == 'train':
        ...
        if args.pred_mode == 'f3wu_Gauss':
            pred_f3wu_Gaussian(args)
        elif args.pred_mode == 'Gauss':
            pred_Gaussian(args)
        elif args.pred_mode == 'Circle':
            pred_Circle(args)
        else:
            raise ValueError("Only ['f3wu_Gauss', 'Gauss', 'SW'] mode is supported.")
    else:
